...
struct timespec start_1min = {0};
struct timespec stop_1min = {0};
struct timespec profile_diff = {0};
int start_flag = 1;
...
static void process_image(const void *p, int size)
{
	...
	if(entry==0)
	{  
	//When the function is called the very first time
	  start = frame_time;
	  entry = 1;
	}
	else
	{
	  ...
	  if(diff_time.tv_nsec > max_time.tv_nsec)
	  max_time = diff_time;
	  
	  printf("frame capture time: %ld secs, %ld ns\n", diff_time.tv_sec, diff_time.tv_nsec);
	  entry=0;
	}
...
static void mainloop(void)
{
    ...
    while (count > 0)
    {
		...
		#ifdef PROFILE_1_MIN
		if(start_flag){
			clock_gettime(CLOCK_REALTIME, &start_1min);
			start_flag = 0;
		}
	
		else{
			clock_gettime(CLOCK_REALTIME, &stop_1min);
		}
		
		profile_diff = diff(start_1min, stop_1min);
		
		if(profile_diff.tv_sec >= 60)
		break;		
    ...
    double avg_rate, worst_rate;
    float jitter;
    avg_rate = (framecnt)/60.0;
    worst_rate = ((double)max_time.tv_nsec/1000000.0);
    jitter = avg_rate - 30.0;
    printf("Average rate (fps) is: %f\n",avg_rate);
    printf("Worst rate(fps) is: %f\n", (1000.0/worst_rate));
	printf("Jitter : %f\n", jitter);
}
...
int main(int argc, char **argv)
{
	...
	printf("Enter S or s for sobel edge detection C or c for Canny edge detection\n");
	
	scanf("%c",&sc);
	printf("\nsc: %c\n", sc);
	
	if((sc != 's') && (sc != 'S') && (sc != 'c') && (sc != 'C')){
		printf("Invalid option !! Selecting sobel as default method !\n");
