...
#define IMG_H_SLICE (IMG_HEIGHT/NUM_ROW_THREADS) // 124
#define IMG_W_SLICE (IMG_WIDTH/NUM_COL_THREADS)  // 145
...
int main(int argc, char *argv[])
{
   ...
   // {
    ...
    for(thread_idx=0; thread_idx<(NUM_ROW_THREADS*NUM_COL_THREADS); thread_idx++)
    {
        ...
        if(thread_idx == 1) {idx=1; jdx=(1*(IMG_W_SLICE-1));}
        if(thread_idx == 2) {idx=1; jdx=(2*(IMG_W_SLICE-1));}
        if(thread_idx == 3) {idx=1; jdx=(3*(IMG_W_SLICE-1));}

        if(thread_idx == 4) {idx=IMG_H_SLICE; jdx=(1);}
        if(thread_idx == 5) {idx=IMG_H_SLICE; jdx=(1*(IMG_W_SLICE-1));}
        if(thread_idx == 6) {idx=IMG_H_SLICE; jdx=(2*(IMG_W_SLICE-1));}
        if(thread_idx == 7) {idx=IMG_H_SLICE; jdx=(3*(IMG_W_SLICE-1));}

        if(thread_idx == 8) {idx=(2*(IMG_H_SLICE-1)); jdx=1;}
        if(thread_idx == 9) {idx=(2*(IMG_H_SLICE-1)); jdx=(1*(IMG_W_SLICE-1));}
        if(thread_idx == 10) {idx=(2*(IMG_H_SLICE-1)); jdx=(2*(IMG_W_SLICE-1));}
        if(thread_idx == 11) {idx=(2*(IMG_H_SLICE-1)); jdx=(3*(IMG_W_SLICE-1));}
